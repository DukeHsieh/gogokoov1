...
const WaitingRoom: React.FC<WaitingRoomProps> = () => {
  ...
  useEffect(() => {
    ...
    // 建立WebSocket連接到房間
    const ws = new WebSocket(`ws://localhost:8080/ws?roomId=${roomId}&nickname=${playerNickname}&isHost=${isHost}`);

    ws.onopen = () => {
      console.log('WebSocket connected.');
      // 可以選擇發送一個加入房間的消息，如果服務器需要的話
      // ws.send(JSON.stringify({ type: 'joinRoom', roomId, nickname: playerNickname }));
    };
