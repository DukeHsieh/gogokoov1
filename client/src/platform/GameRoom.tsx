...
const GameRoom = () => {
  ...
  useEffect(() => {
    ...
    wsManager.addMessageHandler('gameroom-playerJoined', handlePlayerJoined);
    wsManager.addMessageHandler('gameroom-playerListUpdate', handlePlayerListUpdate);
    
    // 主持人不需要顯示在玩家列表中，所以不添加到players狀態
    
    return () => {
      // 清理WebSocket連接和消息處理器
      wsManager.removeMessageHandler('gameroom-playerJoined');
